        ...
        foreach (var assembly in assemblies)
        {
            ...
            var task = Task.Run(async () =>
            {
                try
                {
                    ...
                    var kibanaAcceptAnyCert = Environment.GetEnvironmentVariable("KIB_KIBANA_ACCEPT_ANY_CERT")?.Trim() switch
                    {
                        "1" or "true" => true,
                        "0" or "false" => false,
                        _ => false
                    ...
                    var httpClient = kibanaAcceptAnyCert 
                        ? new HttpClient(new HttpClientHandler() { ServerCertificateCustomValidationCallback = HttpClientHandler.DangerousAcceptAnyServerCertificateValidator }) { BaseAddress = kibanaUri } 
                        : new HttpClient() { BaseAddress = kibanaUri };
                    ...
                    foreach (var ndjsonFile in ndjsonFiles.OrderBy(a => a))
                    {
                        ...
                        var results = await kibanaClient.ImportSavedObjectsAsync(ndjsonFile, attribute.Overwrite, kiboardsId);
                ...
                {
                    messageSink.WriteException("KiBoards failed.", ex);
                }
            });
        }
    }
